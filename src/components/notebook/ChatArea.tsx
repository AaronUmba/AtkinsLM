  ...
  return <div className="flex-1 flex flex-col h-full overflow-hidden">
      {hasSource ? <div className="flex-1 flex flex-col h-full overflow-hidden">
          ...
          <div className="p-4 border-b border-border flex-shrink-0">
            <div className="max-w-4xl mx-auto flex items-center justify-between">
              <h2 className="text-lg font-medium font-heading text-foreground">Chat</h2>
              ...
              <div className="max-w-4xl mx-auto">
                ...
                <div className="bg-card rounded-lg p-6 mb-6 border border-border">
                  {isGenerating ? <div className="flex items-center space-x-2 text-gray-600">
                    ...
                    </div> : <MarkdownRenderer content={notebook?.description || 'No description available for this notebook.'} className="prose prose-gray max-w-none text-foreground leading-relaxed" />}
                </div>
                ...
                {(messages.length > 0 || pendingUserMessage || showAiLoading) && <div className="mb-6 space-y-4">
                    {messages.map((msg, index) => <div key={msg.id} className={`flex ${isUserMessage(msg) ? 'justify-end' : 'justify-start'}`}>
                        <div className={`${isUserMessage(msg) ? 'max-w-xs lg:max-w-md px-4 py-2 bg-primary text-primary-foreground rounded-lg' : 'w-full'}`}>
                          <div className={isUserMessage(msg) ? '' : 'prose prose-gray max-w-none text-foreground'}>
                    ...
                    {showAiLoading && <div className="flex justify-start" ref={latestMessageRef}>
                        <div className="flex items-center space-x-2 px-4 py-3 bg-card rounded-lg border border-border">
    ...
    <div className="flex-1 flex flex-col items-center justify-center p-8 overflow-hidden">
          <div className="text-center mb-8">
            <div className="w-16 h-16 rounded-lg mx-auto mb-4 flex items-center justify-center bg-card border border-border">
              <Upload className="h-8 w-8 text-muted-foreground" />
            </div>
            <h2 className="text-xl font-medium font-heading text-foreground mb-4">Add a source to get started</h2>
      ...
      <div className="p-4 border-t border-border flex-shrink-0">
        <p className="text-center text-sm text-muted-foreground">AtkinsLM can be inaccurate; please double-check its responses.</p>
      </div>
