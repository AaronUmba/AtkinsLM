...
const StudioSidebar = ({
  ...
  if (isEditingMode) {
    return <div className="w-full bg-sidebar border-l border-sidebar-border flex flex-col h-full overflow-hidden">
  ...
  return <div className="w-full bg-sidebar border-l border-sidebar-border flex flex-col h-full overflow-hidden">
      <div className="p-4 border-b border-sidebar-border flex-shrink-0">
        <h2 className="text-lg font-medium font-heading text-sidebar-foreground mb-4">Studio</h2>
        
        {/* Audio Overview */}
        <Card className="p-4 mb-4 border border-sidebar-border bg-card">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-medium font-heading text-card-foreground">Audio Overview</h3>
          </div>
            ...
            /> : <Card className="p-3 border border-gray-200">
              ...
              {currentStatus !== 'generating' && !isGenerating && !isAutoRefreshing && <div className="flex items-center space-x-3 mb-3">
                  <div className="w-8 h-8 rounded flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="hsl(var(--card-foreground))">
                  ...
                  <div className="flex-1">
                    <h4 className="font-medium font-heading text-card-foreground">Deep Dive conversation</h4>
              ...
              {getStatusDisplay() && <div className="flex items-center space-x-2 mb-3 p-2 rounded-md bg-transparent">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-card-foreground">{getStatusDisplay()!.text}</p>
                    <p className="text-xs text-muted-foreground">{getStatusDisplay()!.description}</p>
                  </div>
                </div>}
              
              {/* Audio error div */}
              {audioError && <div className="flex items-center space-x-2 mb-3 p-2 bg-red-900/20 rounded-lg border border-red-700/30">
                  <AlertCircle className="h-4 w-4 text-red-600" />
                  <div className="flex-1">
                    <p className="text-sm text-red-600">Audio unavailable</p>
                  </div>
                  <Button size="sm" variant="outline" onClick={handleAudioRetry} className="text-red-600 border-red-700/30 hover:bg-red-900/20">
              ...
              <div className="flex space-x-2">
                <Button size="sm" onClick={handleGenerateAudio} disabled={isGenerating || currentStatus === 'generating' || !hasProcessedSource || isAutoRefreshing} className="flex-1">
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-medium font-heading text-sidebar-foreground">Notes</h3>
      ...
      <ScrollArea className="flex-1 h-full">
        <div className="p-4">
          {isLoading ? <div className="text-center py-8">
              <p className="text-sm text-sidebar-foreground">Loading notes...</p>
            </div> : notes && notes.length > 0 ? <div className="space-y-3">
              {notes.map(note => <Card key={note.id} className="p-3 border border-sidebar-border hover:bg-sidebar-accent cursor-pointer" onClick={() => handleEditNote(note)}>
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      <h4 className="font-medium font-heading text-sidebar-foreground truncate">{note.title}</h4>
              ...
              <h3 className="text-lg font-medium font-heading text-sidebar-foreground mb-2">Saved notes will appear here</h3>
